...
best_fit_init_memmap(struct Page *base, size_t n) {
    ...
    for (; p != base + n; p ++) {
        ...
        /*LAB2 EXERCISE 2: 2313743*/ 
        // 清空当前页框的标志和属性信息，并将页框的引用计数设置为0
        p->flags = p->property = 0; //与first_fit实现一致  
        set_page_ref(p, 0); 
    }
    base->property = n;
    ...
    if (list_empty(&free_list)) {
        ...
        while ((le = list_next(le)) != &free_list) {
            ...
            // 2、当list_next(le) == &free_list时，已经到达链表结尾，将base插入到链表尾部
            if (base < page) { //与first_fit实现一致
                list_add_before(le, &(base->page_link));
                break;
            } else if (list_next(le) == &free_list) { // base的地址比链表中所有页面都大
...
best_fit_free_pages(struct Page *base, size_t n) {
    ...
    base->property = n; //与first_fit实现一致
    SetPageProperty(base);
    nr_free += n;
    ...
    if (le != &free_list) {
        ...
        if (p + p->property == base) { //与first_fit实现一致
