...
load_icode(int fd, int argc, char **kargv)
{
    ...
    for (int i = 0; i < elf->e_phnum; i++) {
        //(3.2) read raw data content in file and resolve proghdr based on info in elfhdr
        if (load_icode_read(fd, &__ph, sizeof(struct proghdr), elf->e_phoff + i * sizeof(struct proghdr)) != 0) {
            goto bad_cleanup_mmap;
        }
        if (__ph.p_type != ELF_PT_LOAD) {
            continue ;
        }
        if (__ph.p_filesz > __ph.p_memsz) {
            ret = -E_INVAL_ELF;
            goto bad_cleanup_mmap;
        }
        if (__ph.p_filesz == 0) {
        ...
        if (__ph.p_flags & ELF_PF_X) vm_flags |= VM_EXEC;
        if (__ph.p_flags & ELF_PF_W) vm_flags |= VM_WRITE;
        if (__ph.p_flags & ELF_PF_R) vm_flags |= VM_READ;
        // modify the perm bits here for RISC-V
        if (vm_flags & VM_READ) perm |= PTE_R;
        if (vm_flags & VM_WRITE) perm |= (PTE_W | PTE_R);
        if (vm_flags & VM_EXEC) perm |= PTE_X;
        if ((ret = mm_map(mm, __ph.p_va, __ph.p_memsz, vm_flags, NULL)) != 0) {
            goto bad_cleanup_mmap;
        }
        size_t off, size;
        uintptr_t start = __ph.p_va, end, la = ROUNDDOWN(start, PGSIZE);

        ret = -E_NO_MEM;

        end = __ph.p_va + __ph.p_filesz;
        //(3.4) call pgdir_alloc_page to allocate page for TEXT/DATA, read contents in file
        //      and copy them into the new allocated pages
        while (start < end) {
            ...
            // read from file and copy to page
            if ((ret = load_icode_read(fd, page2kva(page) + off, size, __ph.p_offset + (uintptr_t)(start - __ph.p_va))) != 0) {
                goto bad_cleanup_mmap;
            }
            start += size;
        }
        ...
        //(3.5) call pgdir_alloc_page to allocate pages for BSS, memset zero in these pages
        end = __ph.p_va + __ph.p_memsz;
        if (start < la) {
            /* __ph.p_memsz == __ph.p_filesz */
            if (start == end) {
                continue ;
            }
    ...
    // (6) setup uargc and uargv in user stacks
    // setup arguments on user stack following execve stack layout
    uintptr_t stack_top = USTACKTOP;
    uintptr_t stack_ptr = stack_top;

    // reserve space for argv pointers array and argc
    stack_ptr -= sizeof(uintptr_t) * (argc + 2); 
    uintptr_t argv_array = stack_ptr;
    uintptr_t argc_addr = argv_array + sizeof(uintptr_t) * (argc + 1);

    // copy argument strings from top down
    for (int i = argc - 1; i >= 0; i--) {
        int len = strlen(kargv[i]) + 1;
        stack_ptr -= len;
        memcpy((void *)stack_ptr, kargv[i], len);
        ((uintptr_t *)argv_array)[i] = stack_ptr;
    }

    ((uintptr_t *)argv_array)[argc] = 0;
    *(uintptr_t *)argc_addr = argc;
